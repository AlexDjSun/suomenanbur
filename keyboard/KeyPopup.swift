...
class KeyPopup: UIView {
    ...
    init(key: KeyBase) {
        ...
        var adjustX: CGFloat {
            if key.additionalPaddingLeft != 0 {
                return key.basePadding - key.additionalPaddingLeft
            } else if key.additionalPaddingRight != 0 {
                return key.additionalPaddingRight - key.basePadding
            } else {
                return 0
            }
        }
        ...
        let popupX = (popupWidth - key.bounds.width) / 2 + adjustX
        let popupY = (key.basePadding * 2) + (key.bounds.height * 0.75)
    ...
    override func draw(_ rect: CGRect) {
        let keyWidth = key.bounds.width - key.additionalPaddingLeft - key.additionalPaddingRight - key.basePadding * 2
        ...
        drawKeyShape(in: bottomShapeRect, color: .dynamicKeyColor)
        ...
        UIColor.dynamicKeyColor.setFill()  // You can set your desired fill color here
        path.fill()
        ...
        drawKeyShape(in: topShapeRect, color: .dynamicKeyColor)
